...
<div class="container">
	...
	<div style="float:right">
		...
		<p></p>
		<p><b style="color:blue !important">TRANSACTION ID: <br> <span style="color:green !important;font-size: 16px;">{{ $allData['transid']}}</span>  </b></p>
		{{-- <p><b style="color:blue !important">INVOICE ID: <br> <span style="color:green !important;font-size: 16px;">{{ $data['invoice_id']}}</span> </b></p> --}}
		<?php 
		$date =  $allData['created_at'] ?? now()
		?>
	...
	<div class="row">
		<p><b style="color:red">NAME</b><br />
		{{ $allData['participant_name']}}</p>

		<p><b style="color:red">EMAIL ADDRESS</b><br />
		{{ $allData['participant_email']}}</p>

		<p><b style="color:red">PHONE</b><br />
			{{ $allData['participant_phone']}}</p>
	</div>
		...
		<table class="table table-bordered">
			...
			<tbody>
				<tr>
					...
					<td class="col-md-1 text-center">{{ $data['currency'] ?? null}}{{ number_format($allData['programFee']?? 0) }}</td>
					<td class="col-md-2 text-center">{{ $allData['currency'] ?? null }}{{ number_format($allData['amount'] ?? 0)}}</td>
				</tr>
				<tr>
					<td></td>
					<td></td>
					...
					<p><strong>{{ $data['currency']?? null }}{{ number_format($data['amount']?? 0) }}</strong></p>
					</td>
				</tr>
				<tr>
					<td></td>
					...
					<td class="text-center">
					<h4><strong style="color:red !important">{{ $data['currency'] ?? null}}{{ number_format($data['balance'] ?? 0) }}</strong></h4>
					</td>
				</tr>
			</tbody>
		</table>
