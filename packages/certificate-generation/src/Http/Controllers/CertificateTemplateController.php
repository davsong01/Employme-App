...
class CertificateTemplateController extends Controller
{
    ...
    public function create()
    {
        ...
        return view('certificates::templates.edit', $this->formData());
    }
    ...
    public function store(Request $request)
    {
        ...
        $this->syncPrograms($template, $request);
    ...
    private function formData(?CertificateTemplate $template = null): array
    {
        ...
        $programsQuery = Program::where('id', '<>', 1)
            ->orderBy('p_name');

        if ($template) {
            $programsQuery->where(function ($query) use ($template): void {
                $query->whereNull('certificate_template_id')
                    ->orWhere('certificate_template_id', $template->id);
            });
        } else {
            $programsQuery->whereNull('certificate_template_id');
        }

        $programs = $programsQuery->get()->values();
    ...
    private function syncPrograms(CertificateTemplate $template, Request $request): void
    {
        ...
        $programIds = array_keys($assignments);
        ...
        if ($assignments !== []) {
            Program::whereIn('id', $programIds)
                ->get()
                ->each(function (Program $program) use ($template, $assignments): void {
                    $assignment = $assignments[$program->id] ?? null;
                    ...
                    $program->update([
                        ...
                        'auto_certificate_settings' => $settings,
                    ]);
                });
        }
    }
