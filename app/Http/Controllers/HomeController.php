...
class HomeController extends Controller
{
    ...
    public function trainings($id)
    {
        if (checkRoleHas(['Student'])){
            ...
            // $data = TempTransaction::whereRaw('JSON_CONTAINS(program_ids, ?)',[$program->id]
            // )->where('user_id', resolveAuthUser()->id)->first() ?? collect([]);
            $data = getTransactionFromProgramIds($program->id);

            // `getTransactionFromProgramIds` may return a Collection or an object/array.
            // Use `data_get` to safely retrieve values without raising a "property does not exist on collection" error.
            $balance = \data_get($data, 'balance', 0);
            $paid = (\data_get($data, 'currency_symbol') ?? '₦') . number_format(\data_get($data, 'amount', 0));
            $currency_symbol = \data_get($data, 'currency_symbol', '₦');
            $facilitator = \data_get($data, 'facilitator_id');
