...
class PopController extends Controller
{
    ...
    public function getBalance($pop){
        $programTransactions = DB::table('program_user')->where(['user_id' => $pop->user->id, 'program_id' => $pop->program_id])->sum('t_amount');
        $programAmount = $pop->program->e_amount > 0 ? $pop->program->e_amount : $pop->program->p_amount;      
        $balance = $programAmount > $programTransactions ? ($programAmount - $programTransactions) : 0;
        return $balance;
    }
