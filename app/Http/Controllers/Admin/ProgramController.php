    ...
    public function processExportParticipantsDataFromTraining(Request $request)
    {
        ...
        $rows = $transactions->map(function ($tx) use ($programMap) {
            ...
            return [
                ...
                'Email'            => $user?->email,
                'Phone'            => $user?->phone,
                'Payment Type'     => $tx->payment_type ?? $tx->type ?? $payment?->payment_type ?? $payment?->type,
                'Expected Amount'  => $tx->expected_amount ?? $payment?->expected_amount,
                'Amount Paid'      => $tx->amount ?? $payment?->amount,
                'Balance'          => $tx->balance ?? $payment?->balance,
                'Payment Mode'     => $tx->t_type ?? $payment?->t_type,
                'Transaction ID'   => $tx->transid ?? $payment?->transid,
